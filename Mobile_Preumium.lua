do
	...
	toggleButton.Position = UDim2.new(0.10, 0, 0.10, 0)
	toggleButton.Image = "rbxassetid://103126977180359"
...
local AutoSell = Tabs.Main:AddToggle("AutoSell", {
    Title = "Auto Sell",
    Description = "Tự động bán item/cây",
    Default = false
})
...
local AutoTeleport = Tabs.Main:AddToggle("AutoTeleport", {
    Title = "Auto Teleport",
    Description = "Đi thẳng đến mục tiêu quest/quái",
    Default = false
})
AutoTeleport:OnChanged(function(Value)
    _G.Teleport = Value
end)
...
spawn(function()
    while wait(Sec) do
        if _G.Level then
            pcall(function()
                ...
                if questGUI.Visible == false then
                    if _G.Teleport then tp(questInfo[6]) end
                    if (plr.Character.HumanoidRootPart.Position - questInfo[6].Position).Magnitude <= 5 then
                        replicated.Remotes.CommF_:InvokeServer("StartQuest", questInfo[3], questInfo[2])
                    end
                elseif questGUI.Visible == true then
                    local enemyFolder = workspace:FindFirstChild("Enemies")
                    if enemyFolder and enemyFolder:FindFirstChild(questInfo[1]) then
                        for _, v in pairs(enemyFolder:GetChildren()) do
                            if Attack.Alive(v) and v.Name == questInfo[1] and string.find(QuestTitle, questInfo[5]) then
                                repeat
                                    wait()
                                    Attack.Kill(v, _G.Level)
                                until not _G.Level or not v.Parent or v.Humanoid.Health <= 0 or questGUI.Visible == false
                            end
                        end
                    else
                        if _G.Teleport then tp(questInfo[4]) end
                        local targetEnemy = enemyFolder:FindFirstChild(questInfo[1])
                        if targetEnemy and targetEnemy:FindFirstChild("HumanoidRootPart") then
                            if _G.Teleport then tp(targetEnemy.HumanoidRootPart.Position + Vector3.new(0,30,0)) end
                        end
                    end
                end
            end)
        ...
        if _G.Sell then
            pcall(function()
                replicated.Remotes.CommF_:InvokeServer("SellAll")
            end)
        end
    end
end)
	...
	task.spawn(function()
		RunSer.Heartbeat:Connect(function()
			pcall(function()
				...
				if ToolTip == "Script Mascara Cat Hub Premium" then
					if MobAura then
